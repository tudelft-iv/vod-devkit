...
from vod_prediction.eval.prediction.data_classes import Prediction
from vod_prediction.map_expansion.map_api import VODMap
from vod_prediction.prediction import PredictHelper
from vod_prediction.prediction.input_representation.static_layers import load_all_maps
...
def expected_ade_k(
    ...
    """Compute the expected fde over the top k modes."""

    ade = mean_distances(stacked_trajs, stacked_ground_truth)
    ade_prob_sorted = np.array(
        [
            [value, prob]
            for prob, value in sorted(zip(mode_probabilities, *ade), reverse=True)
        ]
    )

    ade_prob_sorted = ade_prob_sorted[: min(k, len(ade_prob_sorted))]

    # Calculate expected error (with normalised probabilities)
    expected_ade = np.dot(ade_prob_sorted[:, 0], ade_prob_sorted[:, 1]) / np.sum(
        ade_prob_sorted[:, 1]
    )
    return expected_ade
...
def expected_fde_k(
    ...
    fde = final_distances(stacked_trajs, stacked_ground_truth)
    fde_prob_sorted = np.array(
        [
            [value, prob]
            for prob, value in sorted(zip(mode_probabilities, *fde), reverse=True)
        ]
    )

    fde_prob_sorted = fde_prob_sorted[: min(k, len(fde_prob_sorted))]

    # Calculate expected error (with normalised probabilities)
    expected_fde = np.dot(fde_prob_sorted[:, 0], fde_prob_sorted[:, 1]) / np.sum(
        fde_prob_sorted[:, 1]
    )
...
class MinFDEK(Metric):
    ...
    def __call__(self, ground_truth: np.ndarray, prediction: Prediction) -> np.ndarray:
        ...
        modes = desired_number_of_modes(results, self.k_to_report)
        return modes
...
class ExpectedADEK(Metric):
    def __init__(self, k_to_report, aggregators: List[Aggregator]):
        """
        Computes the expected average displacement error over the top k predictions.
        :param k_to_report:  Will report the top k result for the k in this list.
        :param aggregators: How to aggregate the results across the dataset.
        """
        super().__init__()
...
class ExpectedFDEK(Metric):
    def __init__(self, k_to_report, aggregators: List[Aggregator]):
        """
        Computes the expected final displacement error over the top k predictions.
        :param k_to_report:  Will report the top k result for the k in this list.
        :param aggregators: How to aggregate the results across the dataset.
        """
        super().__init__()
...
def deserialize_metric(config: Dict[str, Any], helper: PredictHelper) -> Metric:
    ...
    elif config["name"] == "ExpADEK":
        return ExpectedADEK(
            config["k_to_report"],
            [deserialize_aggregator(agg) for agg in config["aggregators"]],
        )
    elif config["name"] == "ExpFDEK":
        return ExpectedFDEK(
            config["k_to_report"],
            [deserialize_aggregator(agg) for agg in config["aggregators"]],
        )

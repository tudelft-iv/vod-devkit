...
from vod_prediction.prediction.helper import angle_of_rotation
...
def smooth_quaternions(quaternions, window_size=7):
    smoothed_quaternions = []
    num_quaternions = len(quaternions)

    # Apply smoothing to each rotation in the sequence
    for i in range(num_quaternions):
        start_index = max(0, i - window_size // 2)
        end_index = min(num_quaternions, i + window_size // 2 + 1)
        window = quaternions[start_index:end_index]

        # Calculate the mean quaternion
        mean_quaternion = np.mean(window, axis=0)
        #  mean_quaternion = Quaternion(mean_quaternion) * Quaternion(
        #      axis=[0, 0, 1], degrees=90
        #  )
        #  mean_quaternion = mean_quaternion.elements
...
def smooth_rotations(annotations):
    quaternions = []
    for sample_annotation in annotations:
        rotation = sample_annotation["rotation"]
        quaternions.append(rotation)

    smoothed_quaternions = smooth_quaternions(quaternions)

    return smoothed_quaternions


def smooth_rotation(helper, i_t, s_t):
    past = helper.get_past_for_agent(
        i_t, s_t, seconds=0.5, in_agent_frame=False, just_xy=False
    )[::-1]
    present = helper.get_sample_annotation(i_t, s_t)
    fut = helper.get_future_for_agent(
        i_t, s_t, seconds=3, in_agent_frame=False, just_xy=False
    )

    present_idx = len(past)

    quaternions = []
    for sample_annotation in past + [present] + fut:
        rotation = sample_annotation["rotation"]
        quaternions.append(rotation)

    smoothed_quaternions = smooth_quaternions(quaternions)

    # smoothed_quaternion = Quaternion(smoothed_quaternions[present_idx])
    return smoothed_quaternions[present_idx]
